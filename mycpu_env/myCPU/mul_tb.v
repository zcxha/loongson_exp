...
module mul_tb;
	...
	always @(posedge mul_clk) begin
		//$random为系统任务，产生一个随机的32位有符号数
		x			<= $random;
		y			<= $random;
		// 加了拼接符，{$random}产生一个非负数，除2取余得到0或1
		mul_signed		<= {$random} % 2;
	end
	...
	// 打印运算结果
	initial begin
		$monitor("x = %x, y = %x, signed = %x, result = %x,OK=%b",
	...
	always @(posedge mul_clk) begin
		if (!ok) begin
			$display("Error : x = %x, y = %x,result = %x, result_ref = %x, OK=%b",
