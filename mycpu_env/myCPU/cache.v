module cache(
    ...
    always @(posedge clk) begin
        if (~resetn) begin
            ...
            if (m_state==IDLE&&valid || m_state==LOOKUP&&cache_hit&&valid&&mat==2'b01) begin
                reg_tag <= tag;
                reg_op <= op;
                reg_index <= index;
                reg_bank <= offset[3:2];
    ...
    assign wr_addr = {sel_tag,reg_index,4'b0};
    ...
    always @(posedge clk) begin
        if (~resetn) begin
            ...
            if (reg_mat==2'b01&&m_state==LOOKUP&&reg_op&&cache_hit) begin
                w_index <= reg_index;
                w_wstrb <= reg_wstrb;
