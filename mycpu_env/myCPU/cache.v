module dcache(
        ...
        input wire [31:0] vaddr, // cacop地址直接索引方式的虚拟地址
    ...
    always @(posedge clk) begin
        if (~resetn) begin
            ...
            if (m_state==IDLE&&valid || !cacop_op&&m_state==LOOKUP&&cache_hit&&valid&&mat==2'b01) begin
                reg_tag <= tag;
                reg_op <= op;
                reg_index <= (cacop_op&&(cacop_code[0]||cacop_code[1])) ? vaddr[11:4] :index;
                reg_bank <= offset[3:2];
                reg_wstrb <= wstrb;
                reg_wdata <= wdata;
                reg_mat <= mat;
    ...
    assign data_ok = (reg_cacop_op&&reg_cacop_code[0]) ? m_state==LOOKUP :
           ...
           (m_state==LOOKUP&&cache_hit || m_state==REFILL&&ret_valid&&n_ret_32==reg_bank)&&reg_mat==2'b01 || (reg_mat==2'b00&&m_state==REFILL&&ret_valid&&ret_last || reg_mat==2'b00&&m_state==REFILL&&reg_op&&wr_rdy);
    ...
    assign aligned_w_wdata = (w_bank==2'b11) ? {w_wdata,96'b0} :
           ...
           {96'b0,w_wdata};
    assign ram_index = (cacop_op&&cacop_code[1]&&m_state==IDLE) ? vaddr[11:4] :
    ...
    wire way1_we;
    wire [7:0] d_index;
    assign d_index = (cacop_op&&cacop_code[1]&&m_state==IDLE) ? vaddr[11:4] :
